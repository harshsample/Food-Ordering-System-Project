...
//                        request.getRequestDispatcher("Admin/adminPage.jsf?faces-redirect=true").forward(request, response);
                            HttpSession session = request.getSession();
                        session.setAttribute("Admin", Email);
                        System.out.println("I am in Admin Role");
                        response.sendRedirect("Admin/adminPage.jsf");
                    }
                    if (result.getCallerGroups().contains("User")) {
                        System.out.println("I am in User Role");
                        HttpSession session = request.getSession();
                    session.setAttribute("User", Email);
                        response.sendRedirect("user/userPage.jsf");
                    }
                    if (result.getCallerGroups().contains("Manager")) {
                        HttpSession session = request.getSession();
                        session.setAttribute("Manager", Email);
                        System.out.println("I am in Manager Role");
                        response.sendRedirect("Manager/managerPage.jsf");
                    }
                    return status;
